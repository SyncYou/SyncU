...
// Mock the sendUserDetails function
...
describe('useLeftFill1', () => {
  ...
  beforeEach(() => {
    // Correctly mock the useUserStore implementation
  ...
  it('should update checked state when setChecked is called', () => {
    ...
    expect(result.current.selectedStack).toEqual(Niches.find((niche) => niche.id === 1));
  });
  ...
  it('should update userDetails when handleAreaClick is called', () => {
    ...
    expect(setUserDetails).toHaveBeenCalledWith('areaOfExpertise', 'Graphics design');
    expect(result.current.isModalOpen).toBe(false);
  });
  ...
  it('should validate the form correctly', () => {
    ...
    // Simulate updating userDetails through the user store
    act(() => {
      setUserDetails(validUserDetails);
    });
  
    // Check if isValid is true after setting areaOfExpertise
    expect(result.current.isValid).toBe(true);
  });

  it('should handle form submission successfully', async () => {
    // Mock sendUserDetails to resolve successfully
    ...
    act(() => {
      setUserDetails(validUserDetails);
      rerender(); // Rerender the hook to reflect changes in userDetails
    });
    ...
    // Verify sendUserDetails was called with the correct arguments
  ...
  it('should handle form submission error', async () => {
    // Mock sendUserDetails to reject with an error
    (sendUserDetails as any).mockRejectedValue(new Error('Failed to send data'));
