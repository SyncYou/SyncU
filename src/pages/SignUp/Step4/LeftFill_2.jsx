        ...
        <>
            <div className="gap-6 self-stretch flex-col " ref={activeRef}>
                ...
                <div className="gap-3 flex-col ">
                    <h1 className="text-[32px] font-semibold text-gray-950 max-sm:text-[24px]">What are you skilled at?</h1>
                    <p className="text-gray-800 text-lg font-normal max-sm:text-base">This will enable us match you to projects that suits you.</p>
                </div>
            </div>

            <div className="gap-4 flex-col relative">
                <h3 className='text-gray-800 font-normal text-sm'>Add atleast 3/10 skills or stacks</h3>
                <span className={`flex items-start justify-between py-2 px-3 border border-solid  h-[90%] bg-white w-[62%] max-sm:w-full rounded-lg ${active ? "border-brand-400 shadow-active-state" : "border-gray-200 max-sm:h-[22vh]"}`}>
                    <span className="flex items-start flex-col gap-1 w-full ">
                        ...
                        {isSearching ? <input type='text' name='skills' className="text-gray-800 text-base font-medium mb-[50px] w-full outline-none" placeholder='Type a skill or stack for e.g, UI design.' value={search} onChange={(e) => setSearch(e.target.value)}
                            ...
                            :
                            <div className="gap-4 max-sm:gap-2 [&_p]:text-gray-950 [&_p]:rounded-3xl [&_p]:border-gray-300 [&_p]:border [&_p]:py-1 [&_p]:px-[20px] w-full flex-wrap [&_p]:max-sm:text-sm">
                ...
                <div className="absolute bottom-[25%] max-sm:bottom-[-7%]" ref={modalRef} > {showModal && <Modal2 filteredSkills={filteredSkills} handleSkillClick={handleSKillClick} />}
                </div>
            </div>

            <div className="gap-2 flex-col">
                <h3 className='text-gray-800 font-normal text-sm'>Suggested skills & stacks</h3>
                <span className="flex flex-wrap  gap-2 [&_p]:text-gray-950 [&_p]:flex [&_p]:items-center [&_p]:gap-[10px] [&_p]:rounded-3xl [&_p]:border-gray-300  [&_p]:border [&_p]:border-solid [&_p]:py-1 [&_p]:px-[20px]  [&_p]:hover:cursor-pointer [&_p]:max-sm:text-sm">
