...
export function User_RightFill({ rightStyle }) {
    ...
    return (
        <>
            <section className={`${location.pathname !== "/finishing" ? "max-sm:hidden" : "max-sm:w-full max-sm:px-[16px]  max-sm:h-full max-sm:pb-[5%] "}  flex flex-col justify-center items-center h-full gap-10  bg-gray-100 py-[100px] pl-[56px] pr-[41px] max-tablet:h-screen`}>
                <div className={` px-[31px] py-[39px] flex flex-col items-center gap-8 border border-gray-200 rounded-3xl bg-white shadow-rightShadow [&_div]:flex [&_div]:items-center  w-full ${rightStyle}`}>
                    <div className='gap-6 flex-col '>
                        ...
                        {location.pathname === "/finishing" ? <span className="w-[284px] flex items-center justify-center bg-gradient-to-r from-[#F77FED] to-[#8D83F9]  font-semibold rounded-full opacity-100 p-1">
                            <Button style="text-[16px] w-full relative border-none bg-gray-950 text-white" onClick={() => setIsLoading(!isLoading)}>
                                <>  <span>Start collaborating</span> <img src={whiteSent} alt="send Icon" /> <img src={whiteShade} alt="send Icon" className='absolute' /></>
                            </Button>
                        </span> :
                            <Button style="text-[16px] w-full text-opacity-40 w-[284px] [&_img]:opacity-40 bg-white cursor-not-allowed">
                                <span>Start collaborating</span> <img src={Send} alt="send Icon" />
                            </Button>}
                    </div>
                    ...
                    <div className='gap-6  w-full flex-col'>
                        <div className=' flex-row justify-between w-full [&_p]:text-gray-800 [&_p]:font-medium [&_p]:text-sm'>
                            ...
                            <p className='text-gray-950 max-tablet:text-right'>{userDetails.location}</p>
                        </div>
                        <div className=' flex-row justify-between w-full [&_p]:text-gray-800 [&_p]:font-medium [&_p]:text-sm '>
                            <span className='flex items-center gap-2'>
                            ...
                            <p className='text-gray-950 max-tablet:text-right'>{userDetails.area || "N/A"}</p>
                        </div>
                        <span className='flex-col flex gap-4 w-full items-start [&_p]:text-gray-800 [&_p]:font-medium [&_p]:text-sm'>
                            ...
                            <div className="gap-4 [&_p]:text-gray-950 [&_p]:rounded-3xl [&_p]:border-gray-300  [&_p]:border [&_p]:py-1 [&_p]:px-[20px] w-full max-tablet:flex-wrap max-tablet:flex-row">
                                {userDetails.stack.map((skill, index) => (<p key={index} className={`${skill === "N/A" ? "border-dashed " : "border-solid shadow-xs"}`}>{skill}</p>))}
                            </div>
                        </span>
                    </div>
                </div>
                {location.pathname === "/finishing" ? <Button onClick={handlePrev} style='rounded-full py-[12px] px-[12px] bg-white shadow-xs '>
                    <BsArrowLeft />
                </Button> : null}
                {/* loading component */}
                {isLoading && <Loading />}
